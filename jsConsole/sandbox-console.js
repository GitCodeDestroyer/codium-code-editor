...
var Sandbox = {
	...
	View : Backbone.View.extend({
		initialize: function(opts) {
			...
			this.resultPrefix = opts.resultPrefix || "&raquo;   ";
			this.tabCharacter = opts.tabCharacter || "\t";
			this.placeholder = opts.placeholder || "// напиши javascript и нажми клавишу enter (console.help() или help() - информация)";
			this.helpText = opts.helpText || " двигаться по истории - клавиши up/down, очистка консоля: `console.clear() и clear()` \n История: `console.history()` \n Префикс для ответа: `sandbox.resultPrefix = '&raquo; '`";
		...
		keydown: function(e) {
			...
			if (e.which === 13) {
				...
				// Run the command past the special commands to check for 'console.help()' and ':clear' etc.
		...
		specialCommands: function(command) {
			if (command === "console.clear()"  || command === "clear()") {
				this.model.destroy();
				return true;
			}
			if ( command === "console.help()" || command === "help()" ) {
				return this.model.addHistory({
					command : 'console.help()',
					result : this.helpText
				});
			}
			if (command === "console.history()" || command === "history()") {
				return this.model.addHistory({
					command : 'console.history()',
					result : JSON.stringify(sandbox.model.get('history'), 0, 4)
				});
			}
			// `:load <script src>`
			if ( command.indexOf(":load") > -1 || command === "load()" ) {
				return this.model.addHistory({
					command : command,
